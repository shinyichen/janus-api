...
janus_error janus_media_templates_helper(const string &data_path, janus_metadata metadata, const size_t min_face_size, const string &templates_dir, const string &templates_list_file, const janus_template_role role, bool verbose)
{
    clock_t start;

    ifstream file(metadata);
    ofstream output(templates_list_file);
    ...
    while (getline(file, line)) {
        ...
        janus_media media;
        janus_template_role template_role;
        ...
        janus_error error = janus_create_template( media, template_role, templates, tracks ); 
        _janus_add_sample(janus_create_template_samples, 1000.0 * (clock() - start) / CLOCKS_PER_SEC);
...
typedef std::vector<float> featv_t;
struct janus_template_type {
  std::vector< featv_t > list_of_image_features;
};

janus_error janus_train_pca_helper(const string &template_list_file, const string &pca_out_file)
{
  std::cout << "Inside train PCA helper" << std::endl;

    // Step 1) Load templates
    vector<janus_template> templates;
    vector<janus_template_id> template_ids;
    vector<int> subject_ids;
    JANUS_CHECK(janus_load_templates_from_file(template_list_file, templates, template_ids, subject_ids));

    std::cout << "Done loading templates" << std::endl;
    std::cout << "Loaded: " << templates.size() << " templates" << std::endl;

    // Step 2) Train PCA
    size_t feat_dim = (*templates[0]).list_of_image_features[0].size();
    size_t total_num_features = 0;

    for (size_t i = 0; i < templates.size(); ++i)
      total_num_features += (*templates[i]).list_of_image_features.size();

    std::cout << "Loaded: " << total_num_features << " features" << std::endl;
    std::cout << "Feature Dim: " << feat_dim << std::endl;

    size_t num_pca_features = total_num_features;
    if (num_pca_features > 4000)
      num_pca_features = 4000;

    cv::Mat data = cv::Mat(num_pca_features, feat_dim, CV_32FC1);
    data.create(num_pca_features, feat_dim, CV_32FC1);
    size_t offset = 0;
    size_t feats_copied = 0;
    for (size_t i = 0; i < templates.size(); ++i) {
      for (size_t j = 0; j < (*templates[i]).list_of_image_features.size() && j < 5 && feats_copied < num_pca_features; ++j) {
	std::cout << "Local feat dim: " << (*templates[i]).list_of_image_features[j].size() << std::endl;
	memcpy(data.data + offset, &((*templates[i]).list_of_image_features[j][0]), sizeof(float)*feat_dim);
	offset += sizeof(float)*feat_dim;
	feats_copied++;
      }
    }

    std::cout << "Done copying features to big matrix" << std::endl;

    cv::Mat W, U, Vt, attributeMean;
    cv::reduce(data, attributeMean, 0, CV_REDUCE_AVG);

    for (size_t r=0; r < data.rows; ++r)
      data.row(r) = data.row(r) - attributeMean;

    std::cout << "Done with mean, about to compute PCA" << std::endl;

    cv::SVD::compute(data, W, U, Vt);

    std::cout << "Done with training, about to save PCA output" << std::endl;

    // Step 3) Write out PCA
    ofstream pca_stream(pca_out_file, ios::out | ios::binary);
    size_t pca_rows = Vt.rows;
    size_t pca_cols = Vt.cols;
    pca_stream.write(reinterpret_cast<const char*>(&pca_rows), sizeof(size_t));
    pca_stream.write(reinterpret_cast<const char*>(&pca_cols), sizeof(size_t));
    pca_stream.write(reinterpret_cast<const char*>(Vt.data), sizeof(float) * pca_rows * pca_cols);
    pca_stream.write(reinterpret_cast<const char*>(attributeMean.data), sizeof(float) * pca_cols /*(Raw Feat Dim)*/);
